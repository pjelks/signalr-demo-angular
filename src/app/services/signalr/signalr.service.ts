...
export class SignalRService {
  ...
  constructor(http: HttpClient, private broadcastService:BroadcastService) {
    this._http = http;
  }

  private getConnectionInfo(): Observable<SignalRConnectionInfo> {
    //let requestUrl = `\${this._baseUrl}negotiate`;
    let requestUrl = `http://localhost:7071/api/negotiate`;
  ...
  init() {
    this.getConnectionInfo().subscribe((info) => {
      ...
      this.hubConnection.on("updated", (data: any) => {
          ...
          this.broadcastService.sendSignalR(data.price);
      });
    });
  }
